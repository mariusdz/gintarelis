/**
 * Gintarėlis – Vercel serverless function
 *
 * Read-only JSON API: /api/news – RSS proxy for the kindergarten news feed.
 * Ported from server.js; logic is identical, only the HTTP wrapper differs.
 */
const http = require("http");
const https = require("https");

// Live news RSS proxy (cached 5 minutes per warm function instance)
let newsCache = null;
let newsCacheTime = 0;
...
module.exports = async function handler(req, res) {
  ...
  // Read-only endpoint: anything beyond GET/HEAD is not supported.
  if (req.method !== "GET" && req.method !== "HEAD") {
    res.setHeader("Allow", "GET, OPTIONS");
    res.status(405).send("Metodas neleidžiamas.");
    return;
  }

  // CDN cache: 5 min fresh, then serve stale while revalidating.
